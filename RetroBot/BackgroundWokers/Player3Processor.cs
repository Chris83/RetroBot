...
{
    class Player3Processor
    {
        private VirtualKeyCode[] controller;

        public Player3Processor(CommandCunk chunk)
        {
            InitController();
        ...
        private void InitController()
        {
            ...
            // Analog Stick
            controller[(int)Key.AnalogUp] = VirtualKeyCode.LSHIFT;
            controller[(int)Key.AnalogRight] = VirtualKeyCode.VK_Z;
            controller[(int)Key.AnalogDown] = VirtualKeyCode.VK_X;
            controller[(int)Key.AnalogLeft] = VirtualKeyCode.VK_C;

            // D Pad
            controller[(int)Key.DPadUp] = VirtualKeyCode.VK_V;
            controller[(int)Key.DPadRight] = VirtualKeyCode.VK_B;
            controller[(int)Key.DPadDown] = VirtualKeyCode.VK_N;
            controller[(int)Key.DPadLeft] = VirtualKeyCode.VK_M;

            // C Buttons
            controller[(int)Key.CUp] = VirtualKeyCode.OEM_COMMA;
            controller[(int)Key.CRight] = VirtualKeyCode.OEM_PERIOD;
            controller[(int)Key.CDown] = VirtualKeyCode.OEM_2;
            controller[(int)Key.CLeft] = VirtualKeyCode.RSHIFT;

            // Triggers
            controller[(int)Key.RTrigger] = VirtualKeyCode.NUMPAD1;
            controller[(int)Key.LTrigger] = VirtualKeyCode.NUMPAD2;
            controller[(int)Key.ZTrigger] = VirtualKeyCode.NUMPAD3;

            // Other Buttons
            controller[(int)Key.Start] = VirtualKeyCode.F10;
            controller[(int)Key.A] = VirtualKeyCode.NUMPAD0;
            controller[(int)Key.B] = VirtualKeyCode.VK_1;
        }
