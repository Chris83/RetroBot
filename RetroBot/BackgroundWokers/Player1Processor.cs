...
namespace RetroBot.BackgroundWokers
{
    class Player1Processor
    {
        ...
        private void InitController()
        {
            ...
            // Analog Stick
            controller[(int)Key.AnalogUp] = VirtualKeyCode.OEM_3;
            controller[(int)Key.AnalogRight] = VirtualKeyCode.TAB;
            controller[(int)Key.AnalogDown] = VirtualKeyCode.VK_Q;
            controller[(int)Key.AnalogLeft] = VirtualKeyCode.VK_W;

            // D Pad
            controller[(int)Key.DPadUp] = VirtualKeyCode.VK_E;
            controller[(int)Key.DPadRight] = VirtualKeyCode.VK_R;
            controller[(int)Key.DPadDown] = VirtualKeyCode.VK_T;
            controller[(int)Key.DPadLeft] = VirtualKeyCode.VK_Y;

            // C Buttons
            controller[(int)Key.CUp] = VirtualKeyCode.VK_U;
            controller[(int)Key.CRight] = VirtualKeyCode.VK_I;
            controller[(int)Key.CDown] = VirtualKeyCode.VK_O;
            controller[(int)Key.CLeft] = VirtualKeyCode.VK_P;

            // Triggers
            controller[(int)Key.RTrigger] = VirtualKeyCode.OEM_4;
            controller[(int)Key.LTrigger] = VirtualKeyCode.OEM_6;
            controller[(int)Key.ZTrigger] = VirtualKeyCode.OEM_5;

            // Other Buttons
            controller[(int)Key.Start] = VirtualKeyCode.NUMPAD7;
            controller[(int)Key.A] = VirtualKeyCode.NUMPAD8;
            controller[(int)Key.B] = VirtualKeyCode.NUMPAD9;
        }
