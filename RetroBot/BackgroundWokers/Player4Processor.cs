...
{
    class Player4Processor
    {
        private VirtualKeyCode[] controller;

        public Player4Processor(CommandCunk chunk)
        {
            InitController();
        ...
        private void InitController()
        {
            ...
            // Analog Stick
            controller[(int)Key.AnalogUp] = VirtualKeyCode.SPACE;
            controller[(int)Key.AnalogRight] = VirtualKeyCode.RCONTROL;
            controller[(int)Key.AnalogDown] = VirtualKeyCode.VK_2;
            controller[(int)Key.AnalogLeft] = VirtualKeyCode.VK_3;

            // D Pad
            controller[(int)Key.DPadUp] = VirtualKeyCode.VK_4;
            controller[(int)Key.DPadRight] = VirtualKeyCode.VK_5;
            controller[(int)Key.DPadDown] = VirtualKeyCode.DIVIDE;
            controller[(int)Key.DPadLeft] = VirtualKeyCode.MULTIPLY;

            // C Buttons
            controller[(int)Key.CUp] = VirtualKeyCode.SUBTRACT;
            controller[(int)Key.CRight] = VirtualKeyCode.VK_6;
            controller[(int)Key.CDown] = VirtualKeyCode.VK_7;
            controller[(int)Key.CLeft] = VirtualKeyCode.VK_8;

            // Triggers
            controller[(int)Key.RTrigger] = VirtualKeyCode.VK_9;
            controller[(int)Key.LTrigger] = VirtualKeyCode.F9;
            controller[(int)Key.ZTrigger] = VirtualKeyCode.OEM_MINUS;

            // Other Buttons
            controller[(int)Key.Start] = VirtualKeyCode.OEM_PLUS;
            controller[(int)Key.A] = VirtualKeyCode.F8;
            controller[(int)Key.B] = VirtualKeyCode.F6;
        }
